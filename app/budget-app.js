  ...
  async function chargeRecurring(item) {
    ...
    try {
      ...
      const row = {
        ...
        credit_card_provider: item.credit_card_provider || null,
        ...
        <Modal title={modal === "recurring" ? (editingRecurring ? "עריכת הוצאה קבועה" : "הוצאה קבועה חדשה") : editingTx ? "עריכת תנועה" : modal === "income" ? "הכנסה חדשה" : "הוצאה חדשה"} onClose={closeModal}>
          {modal === "recurring" ? (
            <form className="form" onSubmit={saveRecurring}>
              ...
              <div className="form-grid"><label>4 ספרות אחרונות<input inputMode="numeric" maxLength="4" value={recForm.credit_card_last4} onChange={(e) => setRecForm({ ...recForm, credit_card_last4: e.target.value.replace(/\D/g, "").slice(-4) })} /></label>{recForm.payment_method === "credit_card" && <label>חברת אשראי<select value={recForm.credit_card_provider} onChange={(e) => setRecForm({ ...recForm, credit_card_provider: e.target.value })}><option value="">לא צוין</option><option value="isracard">ישראכרט</option><option value="cal">כאל</option><option value="max">MAX</option><option value="flycard">Fly Card</option><option value="other">אחר</option></select></label>}</div>
